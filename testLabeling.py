...
def labelPictures():
    if not os.path.isdir("./testSanta"):
        os.makedirs("./testSanta")
    if not os.path.isdir("./testNoSanta"):
        os.makedirs("./testNoSanta")

    folders = os.listdir("./testImages")
    for folderName in folders:
        folderName = "testImages/" + folderName
                ...
                for imgName in images:
                    try:
                        ...
                        if where.lower() == "stop":
                            ...
                            plt.close()
                            title = "./testSanta/Santa_{:09d}.png".format(len(glob.glob('./testSanta/*')))
                            ...
                            plt.close()
                            title = "./testNoSanta/noSanta_{:09d}.png".format(len(glob.glob('./testNoSanta/*')))
                            cv2.imwrite(title,img)
                            print "Saved at %s "%title
